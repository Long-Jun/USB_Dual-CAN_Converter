...
UTL_Timer::UTL_Timer(void)
{
}

int8_t UTL_Timer::every(unsigned long period, void (*callback)(), int repeatCount)
{
...
int8_t UTL_Timer::every(unsigned long period, void (*callback)())
{
	return every(period, callback, -1); // - means forever
}

int8_t UTL_Timer::after(unsigned long period, void (*callback)())
{
	return every(period, callback, 1);
}

int8_t UTL_Timer::oscillate(uint8_t pin, unsigned long period, uint8_t startingValue, int repeatCount)
{
	int8_t i = findFreeEventIndex();
...
int8_t UTL_Timer::oscillate(uint8_t pin, unsigned long period, uint8_t startingValue)
{
 ...
 */
int8_t UTL_Timer::pulse(uint8_t pin, unsigned long period, uint8_t startingValue)
{
 ...
 */
int8_t UTL_Timer::pulseImmediate(uint8_t pin, unsigned long period, uint8_t pulseValue)
{
...
void UTL_Timer::update(void)
{
	unsigned long now = millis();
	update(now);
}

void UTL_Timer::update(unsigned long now)
{
...
int8_t UTL_Timer::findFreeEventIndex(void)
{
	for (int8_t i = 0; i < MAX_NUMBER_OF_EVENTS; i++)
	{
		if (_events[i].eventType == EVENT_NONE)
